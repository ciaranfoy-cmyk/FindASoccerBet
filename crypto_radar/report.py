...
class Report:
    ...
    trending_ids: set = field(default_factory=set)

    def most_talked(self, n: int = 20) -> list[CoinStats]:
        return sorted(self.stats, key=lambda s: (-s.voices, -s.mentions))[:n]
...
def _row(s: CoinStats, trending: set) -> str:
    srcs = "".join(SOURCE_LETTER[k] for k in SOURCE_LETTER if k in s.sources)
    extra = []
    if s.key in trending:
        extra.append("CG-trending")
...
def render_text(report: Report, top: int = 20) -> str:
    ...
    for title, rows in sections:
        ...
        if rows:
            lines.append(HEADER)
            lines.extend(_row(s, report.trending_ids) for s in rows)
        else:
