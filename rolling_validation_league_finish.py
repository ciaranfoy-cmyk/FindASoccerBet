#!/usr/bin/env python3
"""Does average league finishing position (with a 30th-place penalty
for seasons a team wasn't in this specific competition -- see
build_league_finish_features.py) help the model? Built as a division-
aware alternative to Elo, which pools ratings across leagues and so
gives a team promoted from the Championship no penalty at all for the
step up in competition quality. Same clean-swap/combine discipline as
every other feature test here.

    A) baseline    (today's live CORE_CANDIDATES, no finish feature)
    B) finish-only (nothing to swap -- there's no existing equivalent)
    C) combined    (baseline + finish features together)

Usage:
    python3 rolling_validation_league_finish.py
"""
...
FINISH_PATH = "data/league_finish_features.csv"
N_FOLDS = 4
TOP_PCT = 0.05

FINISH_FEATURES = ["home_avg_finish", "away_avg_finish", "avg_finish_gap", "avg_finish_combined"]


def load_data() -> pd.DataFrame:
    df = load_with_player_form_and_shots_venue()
    finish_df = pd.read_csv(FINISH_PATH)[["fixture_id"] + FINISH_FEATURES]
    return df.merge(finish_df, on="fixture_id", how="left")
...
def main() -> None:
    ...
    combined_features = CORE_CANDIDATES + FINISH_FEATURES
    ...
    print("Univariate check: average league finish vs. over/under 2.5")
    univariate_correlations(df, FINISH_FEATURES, "League finish features")

    fold_size = len(model_df) // N_FOLDS
    boundaries = [i * fold_size for i in range(N_FOLDS + 1)]
    boundaries[-1] = len(model_df)

    variants = {"baseline": baseline_features, "finish-only": FINISH_FEATURES, "combined": combined_features}
